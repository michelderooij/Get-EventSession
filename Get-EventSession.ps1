...
param(
    ...
    Function Show-BackgroundDownloadJobs {
        ...
        Write-Progress -Id 2 -Activity 'Background Download Jobs' -Status ('{0} jobs in progress ({1} slidedecks / {2} videos)' -f $Num, $NumDeck, $NumVid) -PercentComplete ($Num / $MaxDownloadJobs * 100)
        ...
        ForEach( $job in $script:BackgroundDownloadJobs) {
            If( $Job.Type -eq 2) {
                ...
                If( $LastLine -match '.*\s(?<pct>[\d\.]+)%.*') {
                    Write-Progress -Id ($progressId++) -Activity ('Video {0}' -f $Job.title) -Status 'Downloading..' -PercentComplete $matches.pct -ParentId 2
                }
                Else {
    ...
    Function Stop-BackgroundDownloadJobs {
        ...
        ForEach( $BGJob in $script:BackgroundDownloadJobs ) { 
            Switch( $BGJob.Type) {
                ...
                2 {
                    Stop-Process -Id $BGJob.job.id -ErrorAction SilentlyContinue
